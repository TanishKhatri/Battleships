...
class Player {
  ...
  placeShip(x, y, shipSize, isHorizontal) {
    ...
    if (isHorizontal && ((x < 0) || ((x+shipSize) > this.gameboard.xBoardLength)) || (y < 0 || y > this.gameboard.yBoardLength - 1)) {
      return "invalidPlacement";
    } else if (!isHorizontal && ((y < 0) || ((y+shipSize) > this.gameboard.yBoardLength)) || (x < 0 || x > this.gameboard.xBoardLength - 1)) {
      return "invalidPlacement";
    }
...
class Computer {
  ...
  placeShipsRandomly() {
    ...
    outerloop: while (currShips.length !== 0) {
      ...
      if (isHorizontal) {
        const x = Math.floor(Math.random()*(this.gameboard.xBoardLength - shipSize + 1));
